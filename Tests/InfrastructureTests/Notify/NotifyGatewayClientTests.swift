...
struct NotifyGatewayClientTests {
    ...
    /// every write, but AgentBar has no decision to make about it, so only the id is read.
    ...
    /// A tile with every field AgentBar drives populated, and a second metric left bare, so one
    ...
    /// A gauge carrying nothing but its identity, for proving the fields AgentBar drives are
    /// stated as explicit nulls rather than quietly left out.
    private func makeTitleOnlyGauge() throws -> NotifyGauge {
        try #require(NotifyGauge(title: "AgentBar"))
    }
    ...
    func `tile start asks for a tile of its own and sends the tile fields`() async throws {
        ...
        #expect(body["title"] as? String == "AgentBar")
    ...
    @Test
    func `gauge body states a null for every field AgentBar drives but has no value for`() async throws {
        ...
        // held. Every field AgentBar owns is therefore stated on every write, as an explicit null
    ...
    func `tile body states a null for a countdown that is no longer known`() async throws {
        ...
        // The fields AgentBar never drives stay unsaid, so a tile can still carry whatever else
    ...
    func `the Home Screen tile and the Live Activity are sent one identical body`() async throws {
        ...
        #expect(screenBody["title"] as? String == "AgentBar")
    ...
    func `a gauge aimed at a browser is sent, because widgets carry no device gate`() async throws {
        ...
        // WB and MC ids can all own one. Refusing this locally would be AgentBar inventing a
    ...
    func `a Home Screen tile aimed at a Mac is sent, because screen widgets carry no device gate`() async throws {
        ...
        // and MC ids can each own one. Refusing it here would be AgentBar inventing a rule the
    ...
    func `endTile treats a 410 as the tile already being gone`() async throws {
        // Given: dismissed, ended or reaped before AgentBar asked
        let link = try makeLink()
    ...
    func `409 on the Home Screen route becomes invalidPayload, not liveActivityUnavailable`() async throws {
        ...
        // and cannot tell which was meant. AgentBar creates its own and addresses it by SW id
    ...
    func `503 is the Home Screen surface being switched off, and worth trying again later`() async throws {
        // Given the server side kill switch this surface shipped behind, so a AgentBar that
        ...
        do {
            ...
            // Then: nothing is wrong with AgentBar, the credentials or the content, and the only
