...
struct NotifySettingsRepositoryTests {
    ...
    /// behaves for a locally built AgentBar: the app is ad-hoc signed, so it has no stable
    ...
    func `a token the Keychain refuses is still stored, and still round trips`() throws {
        // Given a secure store that drops everything, which is what a locally built AgentBar
