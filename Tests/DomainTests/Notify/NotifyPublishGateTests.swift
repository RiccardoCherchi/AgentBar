...
struct NotifyPublishGateTests {
    ...
    private func payload(tile body: String?, gauge value: String?) -> NotifyPayload {
        NotifyPayload(
            tile: body.flatMap { NotifyTile(title: "AgentBar", body: $0, progress: 42) },
            gauge: value.flatMap { NotifyGauge(title: "AgentBar", value: $0, progress: 42) }
        )
    }
    ...
    private func payload(tile body: String?, gauge value: String?, screenTile screenBody: String?) -> NotifyPayload {
        NotifyPayload(
            tile: body.flatMap { NotifyTile(title: "AgentBar", body: $0, progress: 42) },
            gauge: value.flatMap { NotifyGauge(title: "AgentBar", value: $0, progress: 42) },
            screenTile: screenBody.flatMap { NotifyTile(title: "AgentBar", body: $0, progress: 42) }
        )
    }
    ...
    private func payload(tileTrailing: String, gaugeValue: String) -> NotifyPayload {
        NotifyPayload(
            tile: NotifyTile(title: "AgentBar", progress: 42, trailing: tileTrailing),
            gauge: NotifyGauge(title: "AgentBar", value: gaugeValue, unit: "%", progress: 42)
        )
    }
