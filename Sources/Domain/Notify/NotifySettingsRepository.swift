...
public enum NotifyConstants {
    ...
    /// AgentBar that supports it can meet a gateway that is not serving it yet.
...
/// Notify! is not a provider AgentBar reads a quota from, it is a destination
/// AgentBar writes to, so none of the provider vocabulary (enable in the
...
public protocol NotifySettingsRepository: Sendable {
    /// Whether AgentBar publishes to Notify! at all.
    ...
    /// The handle of the Live Activity AgentBar started, so later updates
    ...
    /// The handle of the widget AgentBar created, for the same reason.
    func notifyWidgetId() -> String?
    func setNotifyWidgetId(_ widgetId: String?)

    /// The handle of the Home Screen widget AgentBar created.
