...
/// The credentials that let AgentBar write to one Notify! device: the device
...
public struct NotifyDeviceLink: Sendable, Equatable, Hashable {
    ...
    /// widget, so AgentBar has to know the difference before it writes.
...
/// Which kind of Notify! identity an id names, and therefore what AgentBar can
/// put on it.
///
...
/// and that legacy, `WB` and `MC` devices alike can own one, so AgentBar does
...
/// refused phone looks like AgentBar being broken.
public enum NotifyDeviceKind: Sendable, Equatable, Hashable, CaseIterable {
    ...
    /// rule for AgentBar to invent. A group is excluded because it is not a
    ...
    public var liveActivityUnsupportedReason: String? {
        ...
        case .appDevice, .unrecognized:
            nil
        case .mac:
            "This is a Mac ID. A Live Activity is an iPhone and iPad feature, so AgentBar cannot start one here. The widget still works."
        case .web:
            "This is a browser ID. A Live Activity is an iPhone and iPad feature, so AgentBar cannot start one here. The widget still works."
        case .group:
            Self.groupReason
        }
    }
