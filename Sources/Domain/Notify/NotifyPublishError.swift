    ...
    public var errorDescription: String? {
        ...
        case .liveActivityUnavailable(let reason):
            reason.isEmpty
                ...
                : reason
        case .tileGone:
            "The Live Activity was dismissed on the device. AgentBar will start a new one."
        ...
        case .deliveryUnconfirmed:
            "Notify! could not confirm the Live Activity started. AgentBar will check again on the next update."
        case .transportFailed(let message):
            "Could not reach Notify!: \(message)"
        case .surfaceSwitchedOff(let message):
            message.isEmpty
                ? "Notify! has this widget switched off at the moment. AgentBar will try again later."
                : message
        case .unexpectedStatus(let code):
            "Notify! answered with HTTP \(code)."
        case .malformedResponse:
            "Notify! sent a response AgentBar could not read."
        }
    }
