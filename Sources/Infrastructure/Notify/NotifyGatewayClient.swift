...
/// Writes AgentBar's quota state to the Notify! gateway.
///
...
/// client sends only the fields AgentBar actually drives and never mentions the rest, which is
/// what keeps a AgentBar update from stomping something the user configured elsewhere.
///
/// Routes used:
...
public struct NotifyGatewayClient: NotifyPublishing, Sendable {
    ...
    /// It tells the gateway to start a tile of AgentBar's own instead of taking over whichever
    ...
    public func publishTile(
        ...
        // 400 coming back from a server reads as AgentBar failing at something it should have
        ...
        guard let decoded = try? JSONDecoder().decode(LiveActivityWriteResponse.self, from: data) else {
            AppLog.network.error("Notify!: \(route) answered a body AgentBar could not read")
        ...
        // reports a usable tile token again, so treating it as an error would make AgentBar
    ...
    public func publishScreenTile(
        ...
        do {
            ...
            // placed tile keeps rendering. Nothing is wrong with AgentBar, the credentials or
            ...
            // error's own "switched off at the moment, AgentBar will try again later" wording,
            ...
            // found several screen widgets and cannot tell which one was meant. AgentBar creates
    ...
    public func deviceInfo(link: NotifyDeviceLink) async throws -> NotifyDeviceInfo {
        ...
        guard let decoded = try? JSONDecoder().decode(LinkResponse.self, from: data) else {
            AppLog.network.error("Notify!: GET \(Self.linkRoute) answered a body AgentBar could not read")
            throw NotifyPublishError.malformedResponse
        }
        ...
        guard decoded.type == "device" else {
            throw NotifyPublishError.invalidPayload(
                "That link points at a Notify! group. AgentBar needs a device link, not a group."
            )
        }
    ...
    /// the file a user attaches to a bug report for something AgentBar then
    ...
    static func encode(_ body: [String: Any]) throws -> Data {
        do {
            ...
            throw NotifyPublishError.invalidPayload("AgentBar could not encode this update.")
        }
    }
    ...
    /// the gateway's merge, so staying silent about them is what lets a AgentBar update share a
    ...
    static func tileBody(_ tile: NotifyTile) -> [String: Any] {
        ...
        // Every field AgentBar drives is stated on every write, as an explicit
    ...
    /// but the widget AgentBar creates lives under its `WG…` id until the user removes it, so any
    ...
    /// form is read: the HTTP date form would need a clock AgentBar cannot trust to agree with
...
/// the echoed content is what AgentBar just sent.
...
/// read, for the reason the widget's is: the rest of the object is the content AgentBar just sent
/// back again, plus a `staleAt` the phone owns and AgentBar has no decision to make about.
private struct ScreenWidgetWriteResponse: Decodable {
    let screenWidgetId: String?
}

/// `GET /link`, which answers flat snake_case. Only the four fields AgentBar needs are read, and
