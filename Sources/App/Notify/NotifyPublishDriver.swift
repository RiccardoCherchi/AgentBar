...
final class NotifyPublishDriver {
    ...
    /// the two mean opposite things. Backoff is the gateway asking AgentBar to
    ...
    func publishNow() async -> String? {
        ...
        guard !decision.publishesNothing else {
            ...
            // AgentBar being told to slow down, which the user can sometimes
            ...
            if tileSuppressedUntil != nil {
                return "Notify! is still holding off Live Activity starts. AgentBar will retry on its own."
            }
            return "Notify! is not serving Home Screen widgets yet. AgentBar will try again later."
        }
    ...
    private func report(_ error: any Error, surface: Surface) {
        ...
        case .surfaceSwitchedOff:
            ...
            // switch, so a AgentBar that can write them meets a gateway that is
        ...
        case .transportFailed:
            // The one error whose text is not AgentBar's own. It carries a
