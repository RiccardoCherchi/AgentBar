...
struct NotifyPane: View {
    ...
    private var linkCard: some View {
        SettingsCard {
            ...
            if hasDeviceToken, !tokenIsInKeychain {
                ...
                statusText(
                    "Stored in AgentBar's app credentials rather than the Keychain, which refuses builds that are not signed with a developer identity.",
    ...
    /// explanation sits in another card reads as a bug in AgentBar, while four
    ...
    private func saveLink() {
        ...
        guard hasDeviceToken else {
            verifyOutcome = NotifyActionOutcome(
                message: "Could not store the token on this Mac. Check Console for a Keychain error from AgentBar, and try again.",
                isFailure: true
            )
            return
        }
